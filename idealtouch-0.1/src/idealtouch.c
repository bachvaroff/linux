/*
 * IdealTek serial touchscreen driver (MikroTouch dirver copy)
 *
...
/*
 * 2018/01/22 Atanas Bachvaroff <bachvaroff@gmail.com>
 *   Copied idealtouch.c and adapted for IdealTouch coordinates decoding
 */
...
#define DRIVER_DESC	"IdealTek serial touchscreen driver/idealtouch.c clone"

MODULE_AUTHOR("Atanas Bachvaroff <bachvaroff@gmail.com>");
...
static void mtouch_process_format_tablet(struct mtouch *mtouch)
{
	...
	if (MTOUCH_FORMAT_TABLET_LENGTH == ++mtouch->idx) {
		...
		input_report_abs(dev, ABS_X, cur_x);
		input_report_abs(dev, ABS_Y, cur_y);
		
		input_report_key(dev, BTN_LEFT, mtouch->left);
		input_report_key(dev, BTN_RIGHT, mtouch->right);
...
static void mtouch_process_response(struct mtouch *mtouch)
{
	if (MTOUCH_RESPONSE_END_BYTE == mtouch->data[mtouch->idx++]) {
		...
		printk(KERN_ERR "idealtouch.c: too many response bytes\n");
		mtouch->idx = 0;
	}
}
		...
		unsigned char data, unsigned int flags)
{
	...
	if (MTOUCH_FORMAT_TABLET_STATUS_BIT & mtouch->data[0])
		...
		printk(KERN_DEBUG "idealtouch.c: unknown/unsynchronized data from device, byte %x\n",mtouch->data[0]);
